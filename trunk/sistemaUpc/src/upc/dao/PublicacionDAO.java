...
public class PublicacionDAO extends BaseDAO {
	...
	public Collection<Publicacion> ReportedePublicaciones(Publicacion pbePublicacion) throws DAOExcepcion {
		...
		try {
			con = ConexionBD.obtenerConexion();
			String query = "select cod.Descripcion_Codigo,pu.idpublicacion,pu.titulo,pu.descripcion,pu.fecha_creacion,pu.fecha_publicacion,u.correo,p.nombres,p.paterno,p.materno,cf.nombre as institucion from publicacion pu inner join usuario u on u.idusuario=pu.idusuario inner join persona p on u.idpersona=p.idpersona inner join centro_formacion cf on cf.idcentro_formacion=p.idcentro_formacion join codigo cod on cod.idcodigo=pu.estado and cod.IdGrupo=2 where pu.titulo like ? and (pu.fecha_creacion>=? and pu.fecha_creacion<=?) and pu.estado in("+pbePublicacion.getEstadossel()+") and p.idcentro_formacion="+pbePublicacion.getIdcentro_formacion();
			stmt = con.prepareStatement(query);
